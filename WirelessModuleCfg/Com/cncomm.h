...
class CnComm
{
    public:
        ...
        {
            return iOption == EN_THREAD ? hWatchThread_ : ((iOption == EN_RX_THREAD) ? hReadThread_ :
                    hWriteThread_);
        }
        //! 设置要监视的事件， 打开前设置有效
        ...
        bool Open(DWORD dwPort, DWORD dwBaudRate, BYTE btParity = NOPARITY, BYTE btByteSize = 8,
        ...
        //! 指定的端口名称打开串口
        bool Open(DWORD dwPort, LPCTSTR szPortName, DWORD dwBaudRate, BYTE btParity = NOPARITY,
                  BYTE btByteSize = 8, BYTE btStopBits = ONESTOPBIT)
        {
            ...
            _tcscpy_s(szName_, sizeof(szName_), szPortName);
        ...
        DWORD ReadPort(LPVOID pBuffer, DWORD dwLength, DWORD dwWaitTime = INFINITE)
        {
            ...
            {
                if (dwWaitTime ==
                        INFINITE)//! 如果你没有启用读反冲 直接读取端口将导致阻塞 相当于原来1.3版本 Read(pBuffer, dwLength, false);
                {
        ...
        DWORD Write(char* szBuffer, DWORD dwLength, char* szFormat, ...)
        {
            ...
            _vsnprintf_s(szBuffer, dwLength, _TRUNCATE, szFormat, va);
            va_end(va);
        ...
        DWORD Write(wchar_t* szBuffer, DWORD dwLength, wchar_t* szFormat, ...)
        {
            va_list va;
            va_start(va, szFormat);
            _vsnwprintf_s(szBuffer, dwLength, _TRUNCATE, szFormat, va);
            va_end(va);
        ...
        DWORD Write(wchar_t* szBuffer, wchar_t* szFormat, ...)
        {
            va_list va;
            va_start(va, szFormat);
            vswprintf_s(szBuffer, sizeof(szBuffer) / sizeof(wchar_t), szFormat, va);
            va_end(va);
        ...
        {
            return CN_ASSERT(IsOpen()) &&
                   ::GetCommState(hComm_, pDcb == NULL ? &DCB_ : pDcb) == TRUE ? (pDcb == NULL ? &DCB_ : pDcb) : NULL;
        }
        //! 设置串口参数 DCB
        ...
        //! 设置串口参数：波特率，停止位，等 ***
        bool SetState(DWORD dwBaudRate, BYTE btParity = NOPARITY, BYTE btByteSize = 8,
                      BYTE btStopBits = ONESTOPBIT)
        {
            if (CN_ASSERT(IsOpen()))
            {
        ...
        {
            return CN_ASSERT(IsOpen()) &&
                   ::GetCommTimeouts(hComm_, lpCO ? lpCO : &CO_) == TRUE  ? (lpCO ? lpCO : &CO_) : NULL;
        }
        //! 设置超时
        ...
        DWORD GetQueueCount(bool bInput = true)
        {
            ...
            return CN_ASSERT(IsOpen()) &&
                   ::ClearCommError(hComm_, &dwError, &Stat) ? (bInput ? Stat.cbInQue : Stat.cbOutQue) : (DWORD) - 1L;
        }
        //! 调制解调器 CTS ON
        bool CheckCTS()
        {
        ...
        class BlockBuffer
        {
            public:
                ...
                struct Block
                {
                    ...
                    DWORD           E_;     //!< 结束偏移
                    DWORD
                    S_;     //!< 块大小 内存块最大值不限 内存块最小值由CN_COMM_BUFFER_MIN_BLOCK_SIZE决定
                    Block*          N_;     //!< 下一个块指针
                ...
                DWORD Copy(LPVOID lpBuf, DWORD dwSize, DWORD dwStart = 0)
                {
                    ...
                    for (dwTemp = 0, dwIndex = pCur->B_; dwTemp < dwStart;
                            pCur = pCur->N_, dwIndex = pCur ? pCur->B_ : 0)
                    {
        ...
        static bool Assert(LPCTSTR szMessage)
        {
            ...
            if (dwError)//! 错误代码(GetLastError())不为 0 输出错误描述
            {
                ...
                _sntprintf_s(szMsg + dwLength, 256 - _tcslen(szMsg), _TRUNCATE, _T("Code:%d; "), dwError);
                dwLength = _tcslen(szMsg);
    ...
    protected:
        ...
        HANDLE              hComm_;             //!< 串口句柄
        DWORD
        dwPort_;            //!< 串口号 0为特殊串口 -1为无效 主要终于消息通知甄别
        ...
        virtual bool SetupPort()
        {
            ...
            if (!CN_ASSERT(::SetupComm(hComm_, 4096,
                                       4096))) //! 配置端口发送接收队列大小, 读4096字节, 写4096字节, 阻塞I/O模式发送队列无意义
            {
                return false;
            }
            ...
            CO_.ReadTotalTimeoutConstant    = IsOverlappedMode() ? 500 :
                                              250;//! 读超时 重叠I/O模式下500毫秒 阻塞I/O模式下250毫秒
            CO_.WriteTotalTimeoutMultiplier = IsOverlappedMode() ? 1 : 0;
            CO_.WriteTotalTimeoutConstant   = IsOverlappedMode() ? 10000 :
                                              250;//! 写超时 重叠I/O模式下(10000+1×字节数)毫秒 阻塞I/O模式下250毫秒
            ...
            if (!CN_ASSERT(::PurgeComm(hComm_,
                                       PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR | PURGE_RXCLEAR ))) //! 清除端口
            {
                return false;
            }
        ...
        virtual DWORD OverlappedModel()
        {
            ...
            {
                if (GetQueueCount(
                                        true))//! 等待事件前会先扫描队列是否还有未处理数据 如果有模拟一个EV_RXCHAR事件 避免事件通知的陷阱
                {
